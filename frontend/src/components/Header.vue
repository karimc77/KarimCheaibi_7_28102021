<template>
  <nav
    ...
    class="sticky shadow-lg w-full flex items-center justify-center md:justify-between flex-wrap bg-red-800 h-24 md:h-16 mt-0 pl-8 pr-8 top-0 z-10 animated
       backdrop-filter backdrop-blur-md bg-opacity-70"
  >
    ...
    <div class="flex w-full justify-around md:w-max flex-end">
      ...
      <template v-if="status == 'isConnected'">
        ...
        <router-link
          to="/login"
          @click="logOut()"
          class="text-white font-extrabold hover:text-red-900 mx-3"
