      ...
      <div class="flex items-center">
        ...
        <div class="flex flex-wrap flex-1 items-center">
          <router-link :to="`/profil/${post.User.id}`">
            ...
            <p
              class="text-center font-semibold text-gray-600 hover:text-gray-400"
            >
              {{ post.User.username }}
            </p>
          </router-link>
...
export default {
  ...
  computed: {
    postLiked() {
      // Le bouton like reste en rouge si l'user a déjà liké l'article
      ...
      if (userLike.includes(userId)) {
        return "h-6 w-6 mr-1 text-red-600 ";
      } else {
        return "h-6 w-6 mr-1 text-gray-300 hover:text-red-600";
      }
    },
